    ...
    def tts_with_vc(
        ...
        *,
        ...
        if self.voice_converter is None:
            ...
            # self.load_vc_model_by_name("voice_conversion_models/multilingual/multi-dataset/openvoice_v1")
            # self.load_vc_model_by_name("voice_conversion_models/multilingual/multi-dataset/openvoice_v2")
            # self.load_vc_model_by_name("voice_conversion_models/multilingual/multi-dataset/knnvc")
...
class SUIT_TTS:
    def __init__(self, model_path: str, config_path: str, gpu: bool = False):
        device = "cuda" if gpu else "cpu"
        self.synthesizer = Synthesizer(
            tts_checkpoint=model_path, tts_config_path=config_path, use_cuda=(device == "cuda")
        )
        self.last_embedding = None

        # --- LOAD voice conversion model here --- voice_conversion_models/multilingual/multi-dataset/openvoice_v2
        vc_model = r"C:\Users\msutt\AppData\Local\tts\voice_conversion_models--multilingual--multi-dataset--openvoice_v2\model.pth"
        vc_config_path = r"C:\Users\msutt\AppData\Local\tts\voice_conversion_models--multilingual--multi-dataset--openvoice_v2\config.json"
        self.synthesizer._load_vc(vc_model, vc_config_path, use_cuda=(device == "cuda"))

    def voice_conversion(self, source_wav: str):
        wav, embedding = self.synthesizer.voice_conversion(
            source_wav=source_wav, target_wav=source_wav, return_embedding=True
        )
        # convert to numpy
        if hasattr(embedding, "cpu"):
            embedding = embedding.cpu().numpy()
        self.last_embedding = embedding
        return wav, embedding

    def save_embedding(self, wav_path: str, out_path: str):
        _, embedding = self.voice_conversion(wav_path)
