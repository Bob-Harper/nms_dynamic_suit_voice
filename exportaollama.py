...
def create_ollama_model_file(model_name, output_file, BackUp_Folder, Ollama_Model_Folder):
    ...
    if not template:
        print(
            f"Error: model '{model_name}' not found or the template is empty. Please check the model name and try again.")
        return
...
TEMPLATE """ + '"""' + f"""{template}""" + '"""' + "\n"
    ...
    model_file_location_match = re.search(fr'FROM\s+({re.escape(Ollama_Model_Folder)}[^\s]*)', modelfile_message,
                                          re.MULTILINE)
    extracted_model_file_location = model_file_location_match.group(
        1) if model_file_location_match else "Model_file_location_not_found"
...
Ollama_Model_Folder = r"C:\Users\msutt\.ollama\models"

# Where you want to back up your models:
BackUp_Folder = r"C:\NMS_SUIT_VOICE\assets\qwen3_06b"
# ****************************************************************
# ****************************************************************
# ****************************************************************
